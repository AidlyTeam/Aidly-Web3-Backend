...
class WalletService {
    ...
    async VerifyTransaction(txid: string, donatorWalletAddress: string, campaignWalletAddress: string, recivedSOL: number): Promise<boolean> {
        try {
            ...
            const senderPre = txInfo.meta.preBalances[0];
            const senderPost = txInfo.meta.postBalances[0];
            const fee = txInfo.meta.fee;

            const sentAmountLamports = senderPre - senderPost - fee;

            // SOL → lamports dönüşümü
            const expectedLamports = recivedSOL * 1_000_000_000;
            ...
            const isTransferValid = systemTransfers.some((ix: any) => {
                ...
                return (
                    from === donatorWalletAddress &&
                    to === campaignWalletAddress &&
                    sentAmountLamports === expectedLamports
                );
            });
